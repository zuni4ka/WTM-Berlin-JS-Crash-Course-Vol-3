...
test('User can attend a meetup', async t => {
  const anna = {
   ...
   const createAnnaRes = await request(app)
    .post('/person')
    .send(anna)

   const annaUser = createAnnaRes.body

   const createMeetupRes = await request(app)
    .post('/meetup')
    .send(meetup)
  ...
  const attendMeetupRes = await request(app)
   .post(`/person/${annaUser._id}/meetups`)
   .send({ meetup: createdmeetup._id })

t.is(attendMeetupRes.status, 200)

const alteredAnna = attendMeetupRes.body
